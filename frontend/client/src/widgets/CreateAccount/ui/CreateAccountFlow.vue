...
const { registerUser, errors, isLoading, formValues, isError } = useCreateAccount();
...
<template>
  ...
  <CreateAccountForm @stepUpdate="step = $event" v-if="step == 'create_account'" ref="create_account" :class="step == 'create_account' ? 'translate-x-[0px] duration-[700ms]' : 'opacity-0 translate-x-96 duration-700'" class="relative w-full flex-col justify-start items-center gap-2.5 flex"/>
