  ...
  <Popup :scroll-to-close="true" type="small" :title="'Manage Booking'" :open="showPopup" @close="closePopup">
    ...
    <template #body>
      <div class="flex flex-col gap-7 justify-between items-center relative">
        ...
        <div v-if="getFirstPhoto" class="w-full relative">
          <img :src="getFirstPhoto" :alt="booking.address.company.name" class="w-full max-h-48 object-cover rounded-[10px]"/>
        </div>
        <div class="w-full relative flex justify-start gap-10 items-center">
          <div class="flex justify-start gap-2">
            ...
            <div class="relative flex flex-col gap-0.5">
              ...
              <span :class="`${getRatingColor(booking.address.rating)}`">{{ booking.address.rating }}</span>
            </div>
          </div>
          <div class="flex justify-start gap-2">
    ...
    <template #footer>
      <div class="flex justify-between items-center gap-2 w-full">
        <button @click="cancelBooking()" class="w-full h-11 p-3.5 hover:opacity-90 bg-transparent rounded-[10px] text-red border-red border text-sm font-medium tracking-wide">Cancel Booking</button>
      </div>
    </template>
  </Popup>
</template>
