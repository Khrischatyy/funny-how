...
import {ChooseRole, CreateAccountForm} from "~/src/features/Register/createAccount"
...
import {IconElipse, IconCopyright, IconLine, IconCheck} from "~/src/shared/ui/common/Icon";
...
const { formValues: registrationFormValues, errors: registrationErrors, isLoading: isRegistrationLoading, isError: isRegistrationError, registerUser } = useCreateAccount();
        ...
        <div class="relative w-full flex-col justify-start items-center gap-7 inline-flex">
          ...
          <div class="relative h-full w-full m-2 md:m-0 md:w-96 min-h-auto sm:min-h-[500px] flex justify-start items-center bg-gradient-to-b from-[#000] via-[#000] to-transparent rounded-xl p-5 z-10">
            ...
            <div v-if="step == 'auth' && !session?.isAuthorized" ref="auth" :class="step == 'auth' ? 'translate-x-[0px] duration-[700ms]' : 'opacity-0 -translate-x-96 duration-700'" class="relative w-full flex-col justify-start items-center gap-2.5 flex">
              ...
              <div class="w-full h-7 rounded-lg justify-start items-center gap-2.5 inline-flex">
              ...
              <div class="flex flex-col w-full items-center gap-2.5">
                <button @click="authForm()" class="w-full h-11 hover:opacity-90 bg-white rounded-[10px] text-neutral-900 text-sm font-medium tracking-wide">Sign In</button>

                <div class="justify-center w-full items-center gap-2.5 inline-flex">
                  <button @click="createForm()" class="w-full min-h-11 h-auto p-3.5 hover:opacity-90 rounded-[10px] text-white border text-sm font-medium tracking-wide">Don’t have an account? Create account</button>
                </div>
                <GoogleSignInButton />
              </div>
              <div class="w-full h-11 p-3.5 justify-center items-center gap-2.5 inline-flex">
                <button class="w-full h-11 p-3.5 hover:opacity-90 rounded-[10px] text-white text-sm font-medium tracking-wide">Forgot password?</button>
              </div>
            </div>
