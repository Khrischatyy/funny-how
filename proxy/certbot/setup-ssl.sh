...
# Проверяем и останавливаем существующий контейнер
...
# Используем однократно sudo только для проверки наличия файлов
if sudo test -d ../certbot/conf/live/$DOMAIN/; then
  echo "✅ Сертификаты успешно получены"

  # Запрашиваем sudo только для необходимой операции - изменения прав доступа
  echo "Настраиваем права доступа для сертификатов..."
  sudo chmod -R 755 ../certbot/conf/
  sudo chown -R $(whoami):$(id -gn) ../certbot/conf/
else
  echo "❌ Сертификаты не найдены в ожидаемой директории"
  exit 1
fi
...
echo "docker compose -f prod.yml down"
